...
private:
    ...
    double amount;
public:
    NotSufficientAmountException(double balance, double amount) {
        this->balance = balance;
        this->amount = amount;
    }

    void message() {
        cout << "Operation not allowed. You are trying to withdraw " << amount << " but you have only " << balance
             << endl;
    }
};
...
public:
    ...
    virtual void withdraw(double amount) {
        if (amount > balance) {
            throw NotSufficientAmountException(balance, amount);
        } else {
            balance -= amount;
        }
